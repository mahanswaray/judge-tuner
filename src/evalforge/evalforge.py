...
from src.evalforge.evalforge_alignment import (
...
from src.evalforge.instructor_models import (
...
Your task is to create one assertion that can be used to evaluate LLM outputs based on this criterion. Follow these guidelines:

1. Make the assertion clear, concise, and directly related to the criterion
...
- Provide a clear, detailed prompt for an LLM to evaluate the assertion for any arbitrary input and output
- The prompt should guide the LLM to return "PASS" or "FAIL" based on the evaluation
- The prompt will be later paired with an input and output. So make sure to include enough context in the prompt to evaluate the assertion.
- The annotated data is meant to be an example of the kind of input/output this prompt will be evaluating. Do not directly copy any specific detail of the annotation into the assertion.
...
Ensure that your assertion is directly evaluable and avoid vague or subjective language. Focus on creating assertion that align with human preferences and can be used to validate the quality of LLM-generated evaluations.

Make sure to only generate one assertion. even though it's a list field in the response, always only generate one.
