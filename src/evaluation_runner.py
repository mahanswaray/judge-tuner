...
@weave.op()
def run_evaluations(testcase: Testcase, evaluation_suite: EvaluationSuite) -> EvaluationRunResult:
    wrapped_assertions = [
        (k, [v.assertion]) for k, v in evaluation_suite.assertions.items()
    ]
    scorer = AssertionScorer(
        criterion_assertion_map=CriterionAssertionMap.from_assertions(wrapped_assertions),
    ...
    for criterion, criterion_results in results.items():
        for assertion_name, result in criterion_results.items():
            assertion_content = ""
            if isinstance(evaluation_suite.assertions[criterion].assertion, LLMAssertion):
                assertion_content = evaluation_suite.assertions[criterion].assertion.text # type: ignore
            elif isinstance(evaluation_suite.assertions[criterion].assertion, PythonAssertion):
                assertion_content = evaluation_suite.assertions[criterion].assertion.code # type: ignore
            else:
                raise ValueError(f"Unknown assertion type: {type(evaluation_suite.assertions[criterion].assertion)}")
            assertion_result = {
            ...
            criterion_to_assertion_results[criterion] = AssertionResult(**assertion_result)
            
    return EvaluationRunResult(
        testcase=testcase,
        criterion_to_assertion_results=criterion_to_assertion_results
    )